...
if(isset($_POST['submit']))
{
	...
	if(mysqli_affected_rows($con) == 1)
	{
		if(($lastinsid == $_GET["lins_id"]) && ($_POST['balamts'] == $_POST['paymentamt']))
		{
				...
				$msg= str_replace(" ","%20","Your Loan payment to $txtloanacno has been made on "  . $dts  . " for the amount of Rs. ". $paymentamt. " and Your Loan account has been closed. Thank You for Payment.. -AGH Loans");
				$ch = curl_init();
		...
		else
		{
				...
				$paymenttype  = $_POST['paymenttype'];
				$dts = date("d M Y",strtotime($dt));
				$msg= str_replace(" ","%20","Your payment to $txtloanacno (Installment No. $txtinstno ) has been made on "  . $dts  . " for the amount of Rs. ". $paymentamt. ". Thank You for Payment.. -AGH Loans");
				$ch = curl_init();
				curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Length: 0'));
				curl_setopt($ch,CURLOPT_URL,  "https://www.smsgatewayhub.com/api/mt/SendSMS?APIKey=04024e1f-2d50-4874-b1b5-b1b1d901e928&senderid=BIXTEL&channel=2&DCS=0&flashsms=0&number=$mobno&text=$msg&route=21");
				curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
